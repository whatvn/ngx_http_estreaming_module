...
//static void ngx_http_static_handler(ngx_http_request_t *r);
...
ngx_http_mp4_process(ngx_http_mp4_file_t * mp4) {
    ...
    conf = ngx_http_get_module_loc_conf(mp4->request, ngx_http_estreaming_module);

    mp4->buffer_size = conf->mp4_buffer_size;
...
ngx_http_mp4_read_moov_atom(ngx_http_mp4_file_t *mp4, uint64_t atom_data_size) {
    ...
    if (atom_data_size > mp4->buffer_size) {
        ...
        if (atom_data_size > conf->mp4_max_buffer_size) {
            ngx_log_error(NGX_LOG_ERR, mp4->file.log, 0,
